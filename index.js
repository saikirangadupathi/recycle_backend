...
// Endpoint to get full product details by productId
app.get('/api/products/:productId', async (req, res) => {
  const { productId } = req.params;

  try {
    // Fetch product details by id (string field)
    ...
    res.json(product); // Return the entire product document
  } catch (error) {
...
app.get('/get-eCommerce-OrdersByUsers', async (req, res) => {
  try {
...
app.get('/api/getProductsByIds', async (req, res) => {
  try {
...
// API endpoint to fetch orders by users and pickupIds
app.get('/getOrdersByUsers', async (req, res) => {
  try {
    const userId = req.query.userId;
    const pickupIds = req.query.pickupIds.split(','); // Split the comma-separated string into an array

    let results = [];

    for (let pickupId of pickupIds) {
      const orders = await ScrapOrders.find({
        customerId: userId,
        Id: pickupId
      });

      results = results.concat(orders); // Add all found orders to results without filtering
    }

    // Convert results into a structured object if needed, or return as is
    res.json({ orderslist: results });
  } catch (error) {
    console.error('Failed to fetch orders:', error);
    res.status(500).send('Error fetching orders');
...
app.get('/api/profile', authenticateToken, async (req, res) => {
  try {
    ...
    const { id, name, contactNumber, pickupHistory,addresses,loginCredentials,pickupSavedCart,shoppingSavedCart, reCommerceOrderHistory, couponsHistory,wishlist,paymentMethods ,greenpoints, wallet, recommendedProducts } = user;
    res.json({ id, name, contactNumber, pickupHistory,addresses,loginCredentials,pickupSavedCart,shoppingSavedCart, reCommerceOrderHistory, couponsHistory, wishlist,paymentMethods,greenpoints, wallet, recommendedProducts });
  } catch (error) {
...
app.post('/api/saveCart', authenticateToken, async (req, res) => {
  try {
    const { cart, id } = req.body;  // Destructure cart and id (userId) from request body
    console.log('Received cart:', cart);
    console.log('Received userId:', id);

    if (!cart || !id) {
      return res.status(400).json({ error: 'Cart or userId missing from request body' });
    }

    // Find the user using the provided id
    const user = await user_info.findOne({ id: id });
    ...
    // Map the received cart data to the shoppingSavedCart format
    user.shoppingSavedCart = cart.map(item => ({
...
// Endpoint to add pickup history to a user
app.post('/pickupHistory/add', async (req, res) => {
  const { userId, pickupId, items, pickupDate, status } = req.body;

  // Basic validation
  if (!userId || !pickupId || !pickupDate || !status || !items || !Array.isArray(items)) {
      return res.status(400).json({ message: 'Missing required fields' });
  }

  try {
      // Find user by ID and update their pickup history
      ...
      res.status(201).json({ message: 'Pickup history added successfully', pickupHistory: user.pickupHistory });
  } catch (error) {
      console.error('Error adding pickup history:', error);
...
// Endpoint to add or update an address
app.post('/api/addresses', async (req, res) => {
  const { userId, selectedAddressId, name, addressLine1, addressLine2, city, state, postalCode, country, postOffice, addressType, location } = req.body;

  try {
    // Find the user based on the provided userId from the frontend
    const user = await User.findOne({ 'id': userId }); // Assuming userId is the username. Adjust if necessary.
    ...
    if (selectedAddressId) {
      // Update the existing address with the selectedAddressId
      const addressIndex = user.addresses.findIndex(address => address.addressId === selectedAddressId);
      ...
      user.addresses[addressIndex] = {
        ...user.addresses[addressIndex], // Retain existing fields that are not updated
        name,
        addressId,
        addressLine1,
        addressLine2,
      ...
      // Save the updated user with the modified address
      await user.save();

      return res.status(200).json({ message: 'Address updated successfully', updatedAddress: user.addresses[addressIndex] });
    } else {
      // Create a unique addressId based on the current timestamp
      const addressId = `savedaddressid${Date.now()}`;

      const newAddress = {
        addressId, // Add the generated addressId
        name,
        addressLine1,
        addressLine2,
        city,
        ...
        country,
        postOffice,
        isPrimary: false, // You can adjust this based on your logic
        addressType,
        location,
      };
...
app.delete('/api/addresses/:addressId', async (req, res) => {
  ...
  const { userId } = req.body; // Get userId from the request body or session
  ...
  try {
    ...
    // Find and remove the address from the user's addresses array
    const updatedAddresses = user.addresses.filter(addr => addr.addressId !== addressId);
    ...
    // Update the user's addresses and save the user
...
app.post('/api/wishlist/toggle', async (req, res) => {
  ...
  try {
    ...
    if (wishlistItemIndex > -1) {
      user.wishlist.splice(wishlistItemIndex, 1);  // Remove from wishlist
    } else {
    ...
    const isActive = wishlistItemIndex === -1;  // True if added, false if removed
...
// POST to update reCommerceOrderHistory
app.post('/api/update-users-eCommerceOrders', async (req, res) => {
  const { userId, orderId, productIds, orderDate, deliveryDate, orderValue, paymentMethod, deliveryAddress, orderStatus } = req.body;

  try {
      // Find the user by ID
      const user = await user_info.findOne({ id: userId });
      ...
      // Append the new order history to the user's reCommerceOrderHistory
      user.reCommerceOrderHistory.push(newOrderHistory);
